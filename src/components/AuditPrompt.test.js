...
describe('AuditPrompt', () => {
  it('shows the original stated intention alongside the questions', () => {
    const wrapper = mount(AuditPrompt, { props: { sessionGoalText: '- [x] draft outline' } })
    expect(wrapper.text()).toContain('draft outline')
  })

  it('renders exactly three quick-select options', () => {
    const wrapper = mount(AuditPrompt, { props: { sessionGoalText: '' } })

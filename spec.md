...
| **Audit** | What actually got done? Focused or distracted? Shown alongside the current task list — or discard the block outright |
...
- [ ] "Discard session" at Audit, after a confirmation dialog, writes nothing to `paradone:sessions`, clears the in-flight session, and returns to Setup with the Task List untouched
